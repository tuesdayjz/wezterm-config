--
--[[ FormatItems: Begin ]] ---@class FormatItem.Text
---@field Text string
...
function Cells:update_segment_text(segment_id, text)
  ...
  self.segments[segment_id].items[idx] = {
    Text = text
  }
  return self
end
...
function Cells:update_segment_colors(segment_id, color)
  ...
  if color.bg then
    ...
    if has_bg then
      self.segments[segment_id].items[1] = {
        Background = {
          Color = color.bg
        }
      }
    else
      table.insert(self.segments[segment_id].items, 1, {
        Background = {
          Color = color.bg
        }
      })
  ...
  if color.fg then
    ...
    if has_fg then
      self.segments[segment_id].items[fg_idx] = {
        Foreground = {
          Color = color.fg
        }
      }
    else
      table.insert(self.segments[segment_id].items, fg_idx, {
        Foreground = {
          Color = color.fg
        }
      })
